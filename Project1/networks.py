...
from udp_echo_client import udp_echo_client


def start_workers(server_data):
    """
    Start worker threads for each server in the configuration data.
    """
    threads = []
    stop_event = threading.Event()

    for server in server_data['servers']:
        # thread = threading.Thread(target=worker, args=(server['name'], server['service'], server['interval'], stop_event))
        thread = threading.Thread(target=worker, args=(server['name'], server['service'], server['port'], server['interval'], stop_event))
        thread.start()
...
def worker(server_name, service, port, interval: int, stop_event: threading.Event) -> None:
    """
    Function run by the worker thread.
    Prints a message every 'interval' seconds until stop_event is set.
    """
    while not stop_event.is_set():
        ...
        if service == "ping" or service == "ICMP":
            ping_result = ping(server_name)
            if ping_result is not None:
                addr, ping_time = ping_result
                timestamped_print(f"[{service}] [True] [{addr[0]} - {ping_time:.2f} ms]" if (addr and ping_time is not None) else f"[{server_name}] (ping): Request timed out or no reply received]")
            else:
                print(f"[{server_name}] (ping): Request timed out or no reply received")
        elif service == "traceroute":
            traceroute_result = traceroute(server_name)
            if traceroute_result is not None:
                timestamped_print(f"[{server_name}] Traceroute: ", traceroute_result)
            else:
                print(f"[{server_name}] (ping): Request timed out or no reply received")
        elif service == "HTTP":
            ...
            http_server_status, http_server_response_code, description = check_server_http(server_name)
            timestamped_print(f"[{service}] [{http_server_status}] [URL: {server_name}, Status Code: {http_server_response_code if http_server_response_code is not None else 'N/A'}]")
        elif service == "HTTPS":
            # https://
            if not server_name.startswith("http://") and not server_name.startswith("https://"):
                server_name = "http://" + server_name
            https_server_status, https_server_response_code, description = check_server_https(server_name)
            timestamped_print(f"[{service}][{https_server_status}] [URL: {server_name}, Status Code: {https_server_response_code if https_server_response_code is not None else 'N/A'}, Description: {description}]")
        elif service == "TCP":
            tcp_port_server = server_name
            tcp_port_number = port
            tcp_port_status, tcp_port_description = check_tcp_port(tcp_port_server, tcp_port_number)
            timestamped_print(f"[{service}]  [{tcp_port_status}] [TCP Port: {tcp_port_number}, Description: {tcp_port_description}]")
        elif service == "UDP":
            udp_port_server = server_name
            udp_port_number = port
            udp_port_status, udp_port_description = check_udp_port(udp_port_server, udp_port_number)
            timestamped_print(f"[{service}]  [{udp_port_status}] [Server: {udp_port_server}, UDP Port: {udp_port_number}, Description: {udp_port_description}]")
        elif service == "DNS":
            ...
            for dns_query, dns_record_type in dns_queries:
                ...
                timestamped_print(f"[{service}]  [{dns_server_status}] [Records Results: {dns_query_results}]")
        elif service == "NTP":
            ntp_server_status, ntp_server_time = check_ntp_server(server_name)
            timestamped_print(f"[{service}]  [{ntp_server_status}] [{server_name} is up. Time: {ntp_server_time}]" if ntp_server_status else f"{server_name} is down.]")
        elif service == "ECHO":
            udp_echo_client(server_name, port, "Hello from client!") 
        else:
...
def main() -> None:
    ...
    for server_config in servers:
        ...
        worker_thread: threading.Thread = threading.Thread(target=worker, args=(server_name, service, port, interval, stop_event))
        worker_thread.start()
    ...
    try:
        ...
        # checking treads --> if not fully terminated
        alive_threads = [thread for thread in threading.enumerate() if thread != threading.current_thread()]
        if alive_threads:
            # some threads are still alive after the timeout
            print("Some threads did not terminate within the timeout period.")
        print("Exiting application fully now...")
